...
template<typename T>
std::map<T,T> Dijkstra(const unidirected_graph<T>& G, const T& sorgente, const std::map<unidirected_edge<T>, double>& pesi){
    std::map<T, T> pred;       //predecessore di ogni nodo
    std::map<T, double> dist;  //distanza di ogni nodo dalla sorgente
    ...
    dist[sorgente]=0; //la distanza della sorgente a se stessa è 0
