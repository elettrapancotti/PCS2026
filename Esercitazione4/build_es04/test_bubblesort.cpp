...
int main() {
    ...
    for(int test=0; test< 100; ++test){
        ...
        if(!is_sorted(vec)) { //se is sorted è false allora resituisce errore
            std::cerr<< "Errore: Bubblesort fallisce sui numeri" <<"\n";
            return EXIT_FAILURE;
        }
    }

    std::vector<std::string> parole= {
        "matematica", "italiano", "geografia", "storia", "filosofia", "scienze", "geometria", "fisica", "musica", "informatica"
    };

    bubblesort(parole);

    if(!is_sorted(parole)) {
        std::cerr<< "Errore: Bubblesort fallisce sulle stringhe"<<"\n";
        return EXIT_FAILURE;
    }

    std::cout<<"Tutti i test di Bubblesort passati"<<"\n";
    return EXIT_SUCCESS;
