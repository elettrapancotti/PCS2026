...
int main() {
    ...
    for(int test=0; test< 100; ++test){
        ...
        if(!is_sorted(vec)) { //se is sorted è false allora resituisce errore
            std::cerr<< "Errore: Mergesort fallisce sui numeri" <<"\n";
            return EXIT_FAILURE;
        }
    }
    ...
    mergesort(parole, 0, parole.size()-1);

    if(!is_sorted(parole)) {
        std::cerr<< "Errore: Mergesort fallisce sulle stringhe"<<"\n";
        return EXIT_FAILURE;
    }

    std::cout<<"Tutti i test di Mergesort passati"<<"\n";
    return EXIT_SUCCESS;
